...
class ParseJudgeScoresTest(unittest.TestCase):
    ...
    def test_labels_are_mapped_back_to_their_conditions(self):
        ...
        self.assertEqual(2, by_condition["baseline"]["truth_preservation"])
...
class GraderRubricTest(unittest.TestCase):
    ...
    def test_shipped_rubric_grader_section_never_names_a_condition(self):
        ...
        self.assertIn("truth preservation", section)


class PromptTest(unittest.TestCase):
    CASE = {
        "id": "direct-answer",
        "prompt": "What does `claude --version` print when the installed version is 2.1.263?",
        "criteria": ["Answers 2.1.263."],
        "risk": "low",
    }

    def test_prompt_carries_responses_under_labels_and_never_names_conditions(self):
        responses = {"baseline": "Great question! The answer is 2.1.263.", "candidate": "2.1.263"}
        labels = {"baseline": "B", "candidate": "A"}

        prompt = judge.build_judge_prompt(self.CASE, responses, labels, "SCORE 1-5 PER DIMENSION")

        self.assertIn("Great question! The answer is 2.1.263.", prompt)
        self.assertIn("SCORE 1-5 PER DIMENSION", prompt)
        self.assertIn("Answers 2.1.263.", prompt)
        self.assertIn("truth_preservation", prompt)
...
class NeutralWorkingDirectoryTest(unittest.TestCase):
    def test_runner_cwd_is_fresh_and_outside_the_skill(self):
        # An agent CLI adopts its working directory as project context. Run it
        # inside the skill directory and it starts inspecting the skill instead
        # of answering the prompt, which contaminates the responses graded.
...
class EndToEndTest(unittest.TestCase):
    ...
    def test_judging_produces_paired_rows_the_scorer_accepts(self):
        with tempfile.TemporaryDirectory() as tmp:
                    ...
                    for row in (
                        {
                            ...
                            "condition": "baseline",
                            "runner": "claude",
                            "response": "Great question! The answer is 2.1.263.",
                        },
                        {
                            "case_id": "direct-answer",
                            "trial": 1,
            ...
            self.assertEqual(
                2,
                summary["conditions"]["baseline"]["rows"]
                + summary["conditions"]["candidate"]["rows"],
            )
            self.assertIn("truth_preservation", summary["weights"])
