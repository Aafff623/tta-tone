...
读者成稿正文不靠本脚本插混排；作者手癖表情可以留在金样本那种密度。对话仍按原表抽。

对话收束和对话正文共用同一条**现成的**颜文字 + emoji 混排。默认嵌在对话转折上，不要只贴在回复最后一行。不要手搓 `(・ω・) ✨` 这类输入法里见惯的组合。
...
人情味靠把卡住写清楚、把原因说够；混排是转折上的点到，必须出现在正文里。

- **对话里的成稿 / 讲授 / 答问：** 一则最多一处。默认和热情非禁区时每则都抽，嵌进转折，不要隔一则，也不要只放末尾。读者成稿正文不加。
- **闲聊、简单确认：** 同一则最多两处，可以更松，仍抽自脚本。不要手搓，不要沿路撒，路径和命令里仍不插。
- 同一组合本会话只用一次。
...
同一会话里，同一条混排只用一次。对话里的成稿、讲授、答问不要一则里出现两处。闲聊、简单确认同一则最多两处，第二处仍要 `--exclude` 第一处。不要隔两则再贴同一条。
