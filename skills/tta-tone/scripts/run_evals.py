#!/usr/bin/env python3
"""Validate, run, and score paired response-quality evaluations for tta-tone.

Baseline is the bare task prompt; candidate is the same prompt with the skill
body injected as a response-style instruction. Blind judging lives in judge.py.

Harness design adapted from ayghri/i-have-adhd (MIT): paired conditions,
resumable JSONL outputs, budget guard, and runner isolation.
"""
...
def _neutral_cwd() -> Iterator[str]:
    ...
    this repository and it answers prompts by inspecting the skill instead of
...
def _validate_score(row: dict[str, Any], index: int) -> None:
    ...
    for metric in WEIGHTS:
        ...
        if isinstance(value, bool) or not isinstance(value, (int, float)):
            raise ValueError(f"Score row {index}: {metric} must be a number, got {value!r}")
        if not 1 <= value <= 5:
...
def summarize_scores(scores: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    if candidate["truth_preservation"] < baseline["truth_preservation"] - 0.1:
        reasons.append("Candidate truth preservation regressed by more than 0.1 points.")
...
def _strip_frontmatter(text: str) -> str:
    """Drop a leading YAML frontmatter block from the skill body.

    The injected style instruction is the operative ruleset; frontmatter is
    packaging metadata and must not enter the prompt. A file that opens a
    block but never closes it is left alone rather than emptied.
    """
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    score = subparsers.add_parser("score", help="Aggregate judged score rows")
