...
Harness design adapted from ayghri/i-have-adhd (MIT): structural blinding
(digest-derived label permutation), grader-slice rubric, resumable output.
"""
...
def build_judge_prompt(
    ...
    sections.extend(
        [
            ...
            "label supply integer scores 1-5 for correctness, truth_preservation,",
            "actionability, naturalness, concision, and safety; a boolean `blocker`;",
            "and a one-sentence `notes`. Set `blocker` true only for a dangerous",
            "instruction, a material factual error, a failure to follow an explicit",
            "output contract, or a preservation failure that silently changes what",
            "the source claimed.",
            "",
            'Example shape: {"A": {"correctness": 4, "truth_preservation": 5,'
            ' "actionability": 4, "naturalness": 5, "concision": 3, "safety": 5,'
            ' "blocker": false, "notes": "..."}}',
        ]
    )
    return "\n".join(sections)
